...
export default function BusinessDashboard() {
  ...
  const createBusinessMutation = useMutation({
    mutationFn: (businessData: any) => apiRequest("POST", "/api/business", businessData),
    onSuccess: () => {
      toast({
        title: "Profil bisnis berhasil dibuat!",
        description: "Anda sekarang dapat menambahkan lapangan.",
      });
      queryClient.invalidateQueries({ queryKey: ["/api/business/my"] });
    },
    onError: (error: any) => {
      toast({
  ...
  const updateFacilityMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/facilities/business", business?.id] });
  ...
  const monthlyRevenue = bookings
    .filter((booking: any) => {
      ...
      return bookingMonth === thisMonth && booking.paymentStatus === "paid";
    })
    .reduce((total: number, booking: any) => total + parseFloat(booking.totalPrice), 0);

  const averageRating = facilities.length > 0 
    ? facilities.reduce((sum: number, facility: any) => sum + (facility.averageRating || 0), 0) / facilities.length 
    : 0;
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 mt-2">
              Buat profil bisnis Anda untuk mulai mengelola lapangan olahraga
            </p>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleCreateBusiness} className="space-y-4">
                ...
                >
                  {createBusinessMutation.isPending ? "Membuat..." : "Buat Profil Bisnis"}
                </Button>
              </form>
            </CardContent>
          </Card>
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        {/* Dashboard Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Dashboard Pemilik Usaha</h1>
          <p className="text-gray-600 mt-2">Kelola lapangan dan pemesanan Anda dengan mudah</p>
        </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-500">Pemesanan Hari Ini</p>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-500">Total Lapangan</p>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              >
                ...
                <span className="text-sm font-medium">Tambah Lapangan</span>
              </Button>
              
              <Button className="flex flex-col items-center justify-center h-24 bg-sport-blue hover:bg-blue-700">
                <Calendar className="h-6 w-6 mb-2" />
                <span className="text-sm font-medium">Kelola Jadwal</span>
              </Button>
              ...
              <Button className="flex flex-col items-center justify-center h-24 bg-sport-green hover:bg-green-700">
                ...
                <span className="text-sm font-medium">Lihat Pemesanan</span>
              </Button>
              ...
              <CardContent>
                {facilitiesLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-gray-900">Belum ada lapangan</h3>
                    <p className="mt-1 text-sm text-gray-500">Mulai dengan menambahkan lapangan pertama Anda.</p>
                    <div className="mt-6">
                      <Button
                        onClick={() => setAddFacilityOpen(true)}
                        className="bg-sport-blue hover:bg-blue-700"
                    ...
                    {facilities.map((facility: any) => (
                      <div key={facility.id} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex justify-between items-start">
                          <div className="flex">
                            <img
                              src={facility.images?.[0] || "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=200&h=120"}
                              alt={facility.name}
                              className="w-20 h-16 object-cover rounded-lg"
                            />
                            <div className="ml-4">
                              <h3 className="font-semibold">{facility.name}</h3>
                              <p className="text-sm text-gray-600">{facility.sportType} • Indoor</p>
                              <p className="text-sm text-sport-blue font-medium">
                                Rp {parseFloat(facility.pricePerHour).toLocaleString()}/jam
                              </p>
                              <div className="flex items-center mt-1">
                                ...
                                <span className="text-xs text-gray-500 ml-1">
                                  {facility.averageRating?.toFixed(1) || "N/A"} ({facility.reviewCount || 0} ulasan)
                                </span>
                              </div>
                            </div>
                          </div>
            ...
            <Card>
              ...
              <CardContent>
                {bookingsLoading ? (
                  <div className="space-y-4">
                    {[...Array(3)].map((_, i) => (
                      <div key={i} className="border-l-4 border-gray-200 bg-gray-50 pl-4 py-3 animate-pulse">
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="mt-2 text-sm text-gray-500">Belum ada pemesanan</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {recentBookings.map((booking: any) => (
                      <div
                        ...
                        className={`border-l-4 pl-4 py-3 ${
                          booking.status === "confirmed" ? "border-sport-blue bg-blue-50" :
                          booking.status === "pending" ? "border-sport-orange bg-orange-50" :
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-xs text-gray-600">{booking.facility?.name}</p>
                          ...
                          <Badge 
                            ...
                            className="text-xs"
                          >
                            {booking.status === "confirmed" ? "Konfirmasi" :
                ...
                {recentBookings.length > 0 && (
                  <Button variant="ghost" className="w-full mt-4 text-sport-blue hover:text-blue-700">
          ...
          {selectedFacility && (
            <form onSubmit={handleUpdateFacility} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="edit-price">Harga per Jam</Label>
                  <Input
                    id="edit-price"
                    type="number"
              ...
              <DialogFooter>
                ...
                >
                  {updateFacilityMutation.isPending ? "Menyimpan..." : "Simpan"}
                </Button>
              </DialogFooter>
            </form>
          )}
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent>
          ...
          <p className="text-gray-600">
            Apakah Anda yakin ingin menghapus lapangan "{selectedFacility?.name}"? 
          ...
          <DialogFooter>
            ...
            >
              {deleteFacilityMutation.isPending ? "Menghapus..." : "Hapus"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
