...
export class MemoryStorage implements IStorage {
  ...
  private initializeSampleData() {
    ...
    sampleFacilities.forEach(facility => this.facilities.set(facility.id, facility));
  ...
  async updateUserRole(id: string, role: "customer" | "business"): Promise<User> {
  ...
  async searchFacilities(filters: {
    ...
    for (const facility of Array.from(this.facilities.values())) {
      ...
      // Apply filters
      if (filters.location && !business.address.toLowerCase().includes(filters.location.toLowerCase())) {
        continue;
      }
      
      if (filters.sportType && filters.sportType !== "" && facility.sportType !== filters.sportType) {
        continue;
      }
      ...
      // Calculate rating
      const facilityReviews = Array.from(this.reviews.values()).filter(r => r.facilityId === facility.id);
  ...
  async getBookingsByUserId(userId: string): Promise<(Booking & { facility: Facility; business: Business })[]> {
    ...
    for (const booking of Array.from(this.bookings.values())) {
      if (booking.userId === userId) {
        ...
        const business = facility ? this.businesses.get(facility.businessId) : undefined;
  ...
  async getFacilityRating(facilityId: number): Promise<{ averageRating: number; reviewCount: number }> {
    const facilityReviews = Array.from(this.reviews.values()).filter(r => r.facilityId === facilityId);
